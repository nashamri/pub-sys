...
def toggle_mode(request):
    ...
    current_mode = request.session.get('portal_mode', 'upload')
    new_mode = 'viewer' if current_mode == 'upload' else 'upload'

    # If changing to viewer mode, check if user has editor permission
    if new_mode == 'viewer' and not is_editor(request.user):
        return JsonResponse({
            'status': 'error',
...
def portal(request):
    ...
    if mode == 'viewer':
        if user_is_editor:
            ...
            request.session['portal_mode'] = 'upload'
